...
lruc_error lruc_check_expiration(lruc *cache, uint32_t hash_index) {
  time_t t = get_tick();
  uint32_t i = hash_index /* 0 */, required = 0;
  // check each of the cached items, and the hash table
  lruc_item *item = NULL, *next = NULL, *prev = NULL;
  //if(cache->items) {
  //  for(; i < cache->hash_table_size; i++) {
      item = cache->items[i];
      prev = NULL;
	...
	if (t - item->t >= cache->expiration) {
	  lruc_remove_item(cache, prev, item, i);
	} else {
	  prev = item;
	}
	item = next;
...
// public api
// ------------------------------------------
lruc *lruc_new(uint64_t cache_size, uint32_t average_length, uint32_t expiration) {
  // create the cache
  lruc *cache = (lruc *) calloc(1, sizeof(lruc));
  if(!cache) {
  ...
  cache->items = (lruc_item **) calloc(cache->hash_table_size, sizeof(lruc_item *));
  if(!cache->items) {
...
lruc_error lruc_get(lruc *cache, void *key, uint32_t key_length, void **value) {
  ...
  lruc_check_expiration(cache, hash_index);
